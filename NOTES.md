...
**Shaping de exploración (2026-05-28, `--explore_coef`).** Para combat-minigames con niebla (FindAndDefeatZerglings sobre todo) `beacon_distance` queda inactivo (no hay NEUTRAL) y el reward nativo `+1/kill -1/muerte` no recompensa **explorar** — el agente puede atascarse sin encontrar zerglings. Solución: bonus por **fracción del minimapa ya vista** (`visibility_map > 0`). Definido como `F_explore = explore_coef · (Φ' − Φ)` con `Φ = explored_fraction` (monotónica en el episodio); pure delta, sin γ, así no hay sesgo negativo al estar todo explorado. Se aplica cada step "vivo" (no se gateéa por evento — la exploración no tiene "teleport" análogo al beacon-respawn). Default `--explore_coef 0` (off para minijuegos sin niebla); subirlo a 1.0-5.0 para FindAndDefeatZerglings. Worker manda solo la capa `visibility_map` (no todo el minimap) → IPC marginal.

...
**Estado (dónde se dejó).** Corre y entrena (torch por PyPI, visor en vivo OK), pero **NO aprende — está atascado** (no es solo lento). Confirmado a **~1760 updates / ~28 000 steps / 119 episodios**: el `reward medio(20)` sigue plano en la banda aleatoria (~0.25-0.50, **mejor 2**). La leve oscilación es ruido, no aprendizaje. **Causa:** el marine casi nunca pisa el beacon por azar (mejor 2/episodio) → casi nunca llega el +1 → con recompensa tan escasa los retornos y ventajas son ≈0 → **el gradiente es ≈0 y la política no se mueve de su init aleatorio** (la "trampa de la recompensa escasa"), agravado por un solo env y por re-muestrear destino cada paso (el marine da bandazos y explora mal). Sin bug evidente en la lógica de gradiente (signos/`detach` correctos; el agente se mueve, la selección va, el loss es no-nulo).

**Siguiente paso (REQUERIDO para que aprenda, no opcional).** **Reward shaping por distancia**: recompensa densa pequeña cada step por acercarse al beacon (potential-based, no cambia la política óptima) → da gradiente en cada paso aunque no llegue al beacon → debería aprender en minutos. Doble función: si con shaping aprende rápido, confirma que era la recompensa escasa (no un bug); si ni así, hay bug que cazar. Opcional adicional: más exploración (`--entropy`) o varios envs en paralelo (rompe el "ver uno en vivo"). Techo de referencia: el agente scripted `pysc2.agents.scripted_agent.MoveToBeacon` (vía `live_view --agent`) resuelve el mapa (~25/episodio); el aleatorio (~1) es el suelo.
